...
async def test_emotion_patterns():
    ...
    try:
        ...
        for i, emotions in enumerate(emotion_tests):
            ...
            if pattern.steps:
                print(
                    f"Generated pattern: {len(pattern.steps)} steps, {pattern.repeat_count} repeats"
                )
                for j, step in enumerate(pattern.steps):
                    print(
                        f"  Step {j+1}: intensity={step.intensity:.2f}, duration={step.duration}ms"
                    )
                
                # Send pattern to Arduino
                print("📳 Sending pattern to Arduino...")
                success = await controller.send_pattern(pattern)
                if success:
...
async def test_mcp_server():
    ...
    print(
        "4. send_arduino_vibration({'pattern_type': 'pulse', 'intensity': 0.7, 'duration_ms': 1000, 'repeat_count': 2})"
    )
