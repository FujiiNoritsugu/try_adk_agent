...
class LeapMotionHTTPAgent(Agent):
    ...
    def __init__(self, *args, **kwargs):
        # Leap Motion関連の設定を抽出
        leap_server_url = kwargs.pop(
            "leap_server_url",
            os.getenv("LEAP_MOTION_SERVER_URL", "http://192.168.43.162:8001"),
        )
    ...
    def start_leap_polling(self):
        ...
        if not self.is_polling:
            ...
            object.__setattr__(
                self,
                "polling_thread",
                threading.Thread(target=self._polling_loop, daemon=True),
            )
            ...
            while self.is_polling:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if (
                            not data.get("mock", False)
                            and data.get("gesture_type") != "none"
                        ):
                            # 最小間隔チェック
                            ...
                            if (
                                ...
                                >= self.leap_min_process_interval
                            ):
                                ...
                                touch_input = TouchInput(
                                    ...
                                    hand_position=(
                                        data.get("raw_leap_data", {}).get(
                                            "hand_position"
                                        )
                                        if data.get("raw_leap_data")
                                        else None
                                    ),
                                    hand_velocity=(
                                        data.get("raw_leap_data", {}).get(
