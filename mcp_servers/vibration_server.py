...
# Add parent directory to path to import vibration_sensor_controller
...
arduino_controller: Optional[VibrationSensorController] = None
...
async def control_vibration(arguments: ControlVibrationArgs) -> List[TextContent]:
    ...
    if arduino_controller is None or not arduino_controller.is_connected:
        # 接続試行
        arduino_controller = VibrationSensorController("vibration_sensor")
...
async def initialize_arduino() -> List[TextContent]:
    """Arduinoセンサーを初期化します"""
    global arduino_controller
    ...
    try:
        ...
        if connected:
            ...
            return [TextContent(
                ...
                text=json.dumps({
                    "success": True,
                    "message": "Arduinoセンサーの初期化に成功しました",
...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Arduinoセンサーを初期化・キャリブレーションします",
            inputSchema={},
        )
    ]
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "generate_vibration_pattern":
        ...
        return await initialize_arduino()
    else:
